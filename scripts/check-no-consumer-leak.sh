...
#
# CI guard: sentinel (an onlytool) must never depend on or be aware of vykean (a
...
if [[ -n "$hits" ]]; then
  ...
  echo ""
  echo "Found vykean references in sentinel source — onlytools must not be aware of consumers."
  echo "Architectural rule: vykean depends on sentinel; sentinel must not depend on vykean."
  echo ""
  echo "$hits"
  exit 1
fi
